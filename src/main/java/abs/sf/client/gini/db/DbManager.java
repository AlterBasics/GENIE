...
/**
 * Manages database operations. Encapsulates database instance
 * and it's one point stop for all the database operations.
 */
public final class DbManager {
    ...
    private DbManager() {
        Context ctx = (Context) Platform.getInstance().getSession().get(ContextProvider.KEY_CONTEXT);
    ...
    /**
     * Closes database helper and database instance held.
     */
    public void close() {
        dbHelper.close();
    }
    ...
    /**
     * Fetch all the active conversations. The method join on {@link RosterTable}
     * to extract peer name (user/group). All the other required details are directly
     * taken from {@link ConversationTable}.
     */
    public List<Conversation> fetchConversations() {
        List<Conversation> conversations = this.dbHelper.query(SQLQuery.FETCH_CONVERSATIONS, new ConversationRowMapper());
        populatePresence(conversations);
        return conversations;
    }

    private void populatePresence(List<Conversation> conversations) {
    ...
    /**
     * Add a conversation into {@link ConversationTable}. The method does not check
     * if there is already an entry against a JID. Therefore the caller must ensure
     * that there is no pre-existing entry in the table; as there can be only one
     * running conversation with a given JID.
     *
     * @param conv {@link Conversation} to be added into table
     */
    public void addConversation(Conversation conv) {
        ContentValues contentValues = new ContentValues();
        contentValues.put(ConversationTable.COLUMN_PEER_JID, conv.getPeerJid());
        contentValues.put(ConversationTable.COLUMN_LAST_CHATLINE, conv.getLastChatLine());
        contentValues.put(ConversationTable.COLUMN_LAST_CHATLINE_TYPE, conv.getLastChatLineType().name());
        contentValues.put(ConversationTable.COLUMN_LAST_UPDATE_TIME, DateUtils.currentTimeInMiles());
        contentValues.put(ConversationTable.COLUMN_UNREAD_CHATLINE_COUNT, conv.getUnreadChatLines());

        dbHelper.insert(ConversationTable.TABLE_NAME, contentValues);
    }

    /**
     * Updatting conversation
     *
     * @param conv
     */
    public void updateConversation(Conversation conv) {
        ...
        dbHelper.update(ConversationTable.TABLE_NAME, contentValues, ConversationTable.COLUMN_PEER_JID + "= ?", new String[]{conv.getPeerJid()});
    }

    /**
     * Update values for an existing Conversation in {@link ConversationTable}.
     * The method does not check if the conversation exist in the table; therefore
     * Caller must ensure that there such entry exist in the table.
     *
     * @param line
     */
    public void addOrUpdateConversation(ChatLine line) {
        synchronized (ConversationTable.class) {
            if (DbManager.getInstance().conversationExists(line.getPeerBareJid())) {
                Conversation conv = new Conversation(line);

                if (line.getDirection() == ChatLine.Direction.RECEIVE) {
                    int unread = dbHelper.queryInt(SQLQuery.FETCH_UNREAD_CHATLINE_COUNT, new String[]{line.getPeerBareJid()});
                    conv.setUnreadChatLines(unread + 1);
                }

                DbManager.getInstance().updateConversation(conv);
    ...
    public void updateUnreadConversationCount(String peerJid, int unreadConversationCount) {
        ...
        dbHelper.update(ConversationTable.TABLE_NAME, contentValues, ConversationTable.COLUMN_PEER_JID + "= ?", new String[]{peerJid});
    }

    /**
     * Check if there an active conversation with a given {@link JID}. If there is an
     * entry found inside {@link ConversationTable}, it is assumed that there exist an
     * active conversation with the given JID
     *
     * @param peerJid
     * @return
     */
    public boolean conversationExists(String peerJid) {
        long count = dbHelper.queryInt(SQLQuery.FETCH_CONVERSATION_COUNT, new String[]{peerJid});
    ...
    public void updateDeliveryStatus(String messageId, ChatLine.MessageStatus messageStatus) throws SQLException {
        ...
        dbHelper.update(ChatStoreTable.TABLE_NAME, contentValues, ChatStoreTable.COL_MESSAGE_ID + "= ?", new String[]{messageId});
    }
...
//          Long uuid = dbHelper.queryLong(SQLQuery.FETCH_MESSAGE_UUID, new String[]{messageId});
        //TODO: For now testing purpose we are not markinng all reads. In future afetr all set we will enable it.
//        ContentValues contentValues = new ContentValues();
//        contentValues.put(ChatStoreTable.COL_DELIVERY_STATUS, ChatLine.MessageStatus.RECEIVER_HAS_VIEWED.getValue());
//
//        dbHelper.update(ChatStoreTable.TABLE_NAME, contentValues, ChatStoreTable.COL_UUID + " <= ? AND "
//                + ChatStoreTable.COLUMN_PEER_JID + " = ?", new String[]{uuid.toString(), pearJID});
    }
    ...
    public List<ChatLine> getAllUnreadChatLines(String pearJID) {
        return this.dbHelper.query(SQLQuery.FETCH_UNREAD_CONVERSATION_CHAT_LINES, new String[]{pearJID}, new ChatLineRowMapper());
    }

    /**
     * Fetch all the {@link ChatLine} stored in {@link ChatStoreTable} for a conversation
     * based on peer {@link JID}
     *
    ...
    public List<ChatLine> fetchConversationChatlines(String pearJID, boolean isGroup) {
        List<ChatLine> chatLines = this.dbHelper.query(SQLQuery.FETCH_CONVERSATION_CHAT_LINES, new String[]{pearJID}, new ChatLineRowMapper());
    ...
    /**
     * Fetch all the chatlines which have not been delivered to server as yet. A {@link ChatLine}
     * with delivery status 0 indicates undelivered chatline.
     *
     * @return
     */
    ...
    /**
     * Add a roster item into {@link RosterTable}. The method does not check
     * if there is already an entry for the same JID. Therefore caller must
     * ensure that it's adding an item which does not exist in the table already
     *
     * @param item
     */
    ...
    /**
     * Update an existing {@link RosterItem}. The method does not check
     * if the item already exist in the table. Therefore caller must ensure
     * that item is present in the table.
     *
     * @param item
     */
    public void updateRosterItem(RosterItem item) {
        ...
        dbHelper.update(RosterTable.TABLE_NAME, contentValues, RosterTable.COLUMN_JID + " = ?", new String[]{item.getJid().getBareJID()});
    }
    ...
    public void addOrUpdateRosterItem(RosterItem item) {
        synchronized (RosterTable.class) {
            long count = dbHelper.queryInt(SQLQuery.FETCH_ROSTER_ITEM_COUNT, new String[]{item.getJid().getBareJID()});
    ...
    public void addOrUpdateChatRoom(ChatRoom chatRoom) {
        synchronized (RosterTable.class) {
            long count = dbHelper.queryInt(SQLQuery.FETCH_CHAT_ROOM_COUNT, new String[]{chatRoom.getRoomJID().getBareJID()});
    ...
    public void addChatRoom(ChatRoom chatRoom) {
        ...
        contentValues.put(RosterTable.COLUMN_ACCESS_MODE, chatRoom.getAccessMode() == null ? ChatRoom.AccessMode.PUBLIC.val() : chatRoom.getAccessMode().val());
    ...
    public void updateChatRoomSubject(String roomJID, String subject) {
        ...
        dbHelper.update(RosterTable.TABLE_NAME, contentValues, RosterTable.COLUMN_JID + " = ?", new String[]{roomJID});
    }
    ...
    public void updateChatRoom(ChatRoom chatRoom) {
        ...
        dbHelper.update(RosterTable.TABLE_NAME, contentValues, RosterTable.COLUMN_JID + " = ?", new String[]{chatRoom.getRoomJID().getBareJID()});
    }
    ...
    public ChatRoom getChatRoomDetails(String roomJID) {
        ChatRoom room = dbHelper.queryForObject(SQLQuery.FETCH_CHAT_ROOM_DETAILS, new String[]{roomJID}, new ChatRoomRowMapper());
    ...
    public void updatePresence(String userJID, PresenceType presence, String mood, String status) {
        ...
        dbHelper.update(PresenceTable.TABLE_NAME, contentValues, PresenceTable.COLUMN_JID + " = ?", new String[]{userJID});
    }
    ...
    public boolean isOnline(String userJID) {
        String presence = dbHelper.queryString(SQLQuery.FETCH_PRESENCE_STATUS, new String[]{userJID});
    ...
    public UserPresence getPresenceDetails(String userJID) {
        return dbHelper.queryForObject(SQLQuery.FETCH_PRESENCE_DETAILS, new String[]{userJID}, new PresenceRowMapper());
    }
    ...
    public void addOrUpdateUserProfileData(UserProfileData userProfileData) {
        synchronized (UserProfileTable.class) {
            long count = dbHelper.queryInt(SQLQuery.FETCH_USER_PROFILE_COUNT, new String[]{userProfileData.getJabberId().getBareJID()});
    ...
    private long addUserProfile(UserProfileData userProfileData) {
        ...
        if (userProfileData.getAvtar() != null) {
            ...
            if (StringUtils.isNullOrEmpty(avtar.getBase64EncodedImage())) {
                ...
                contentValues.put(UserProfileTable.COLUMN_AVATAR, Base64.decode(avtar.getBase64EncodedImage(), Base64.DEFAULT));
            }
    ...
    public long updateUserProfile(UserProfileData userProfileData) {
        ...
        return dbHelper.update(UserProfileTable.TABLE_NAME, contentValues, UserProfileTable.COLUMN_JID + " = ?", new String[]{userProfileData.getJabberId().getBareJID()});

    }

    public UserProfileData getUserProfileData(String userJID) {
        UserProfileData userProfileData = dbHelper.queryForObject(SQLQuery.FETCH_USER_PROFILE_DATA, new String[]{userJID}, new UserProfileRowMapper());
        return userProfileData;
    }

    public byte[] getUserAvatarBytes(String userJID) {
        return dbHelper.queryForObject(SQLQuery.FETCH_USER_PROFILE_AVATAR, new String[]{userJID}, new RowMapper<byte[]>() {
    ...
    public void updateMediaPath(String mediaId, String mediaPath) {
        ...
        dbHelper.update(MediaStoreTable.TABLE_NAME, contentValues, MediaStoreTable.COL_MEDIA_ID + " = ?", new String[]{mediaId});
    }

    public MediaContent getMediaDetaisByMediaId(String mediaId) {
        return dbHelper.queryForObject(SQLQuery.FETCH_MEDIA_DETAILS_BY_MEDIA_ID, new String[]{mediaId}, new MediaRowMapper());
    }

    public MediaContent getMediaDetaisByMediaUUID(Long uuid) {
        return dbHelper.queryForObject(SQLQuery.FETCH_MEDIA_DETAILS_BY_UUID, new String[]{uuid.toString()}, new MediaRowMapper());
    }
    ...
    public List<Stanza> fetchAllUndeliverStanzas() {
        synchronized (UndeliverStanzaTable.class) {
            List<Stanza> stanzas = dbHelper.query(SQLQuery.FETCH_ALL_UNDELIVERD_STANZAS, new UndeliverStanzaRowMapper());
